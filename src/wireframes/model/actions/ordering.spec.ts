...
describe('OrderingReducer', () => {
    ...
    it('should move items', () => {
        ...
        expect(state_2.diagrams.get(diagram.id)?.itemRootIds.values).toEqual([shape2.id, shape1.id, shape3.id]);
    });
    ...
    function testOrdering(mode: OrderMode, shape: DiagramItem, expectedIds: string[]) {
        ...
        expect(state_2.diagrams.get(diagram.id)?.itemRootIds.values).toEqual(expectedIds);
    }
});
