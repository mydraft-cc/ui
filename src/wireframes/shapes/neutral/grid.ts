...
export class Grid implements ShapePlugin {
  ...
  public render(ctx: RenderContext) {
    ...
    const { rows, columnCounts } = this.parseText(ctx.shape);

    const cellHeight = h / rows.length;

    this.createFrame(ctx);
    this.createHeader(ctx, cellHeight);
    this.createBorders(ctx, columnCounts, w, h, rows, cellHeight);
    this.createTexts(rows, columnCounts, cellHeight, ctx);
  }
  ...
  private createTexts(
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const cellWidth = ctx.rect.width / columnCount;
  ...
  private createBorders(
    ...
    const strokeWidth = ctx.shape.getAppearance(
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      for (let x = 1; x < columnCount; x++) {
        ...
        const rect = new Rect2(offset, y, strokeWidth, cellHeight);
  ...
  private createHeader(ctx: RenderContext, height: number) {
    ...
    ctx.renderer2.roundedRectangleTop(
      ctx.shape,
      CommonTheme.CONTROL_BORDER_RADIUS,
      rect,
      (p) => {
        if (!ctx.shape.getAppearance(HEADER_HIDDEN)) {
          p.setBackgroundColor(
            ctx.shape.getAppearance(HEADER_BACKGROUND_COLOR)
          );
        }
  ...
  private parseText(shape: Shape) {
    ...
    if (!result || result.key !== key) {
      ...
      result = { parsed: { rows, columnCounts }, key };
...
type Parsed = { rows: string[][]; columnCounts: number[] };
